...
{
    // position fixe de chaque case dans la matrice Pl[11][30]
    ...
    // copier le plateau de base
    for(i = 0; i < 11; i++)
        strcpy(grille[i], P->Pl[i]);

    // placer les pions par coordonnées directes
    PlacerPionPlateau(P,3,J2);
    PlacerPionPlateau(P,9,J1);
    PlacerPionPlateau(P,12,J1);
    PlacerPionPlateau(P,23,J2);
    PlacerPionPlateau(P,16,J2);
    PlacerPionPlateau(P,6,J1);
    ...
    for(i = 0; i < Nbr_Cases; i++)
    {
        if(P->Case[i] == J1)
        {
            printf(BLK);
            printf(BG_YEL);
            printf("%c",grille[posLigne[i]][posCol[i]]);
            printf(COLOR_RESET);
        }
        else if(P->Case[i] == J2)
        {
